...
void IndexSSG::Build(size_t n, const float *data,
  ...
  for (size_t i = 0; i < nd_; i++) {
    ...
    if (size > cluster_num) {
      ...
      nc_[i].resize(cluster_num);
    

      for (size_t j = 0; j < cluster_num; j++) {  //选取数据集中距离类中心最近的点作为新的类中心
        float min_dist = -1;
        unsigned* new_cluster_center = new unsigned[cluster_num];
        std::memset(new_cluster_center, 0, sizeof(unsigned) * cluster_num);
        for (size_t k = 0; k < size; k++) {
          if (labels[k] == j) {
            float temp = distance_ -> compare(data_ + final_graph_[i][k] * dimension_, m_means[j], dimension_);
            if (temp < min_dist || min_dist == -1) {
              min_dist = temp;
              new_cluster_center[j] = final_graph_[i][k];
            }
          }
        }
        nc_[i][j].push_back(new_cluster_center[j]); //新类中心作为该类的第一个元素
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      for (size_t i = 0; i < cluster_num_; i++) {
        float temp = distance_ -> compare(query, (const float*)(opt_graph_ + *(neighbors + cluster_offset) * node_size + sizeof(float)), (unsigned)dimension_);
...
float IndexSSG::GetLabel(const float* sample, int* label)	//返回sample到所有类中心的最小距离
{
	float dist = -1;
