...
void IndexSSG::InitRangeProportion(const float rr_p) {
...
void IndexSSG::Build(size_t n, const float *data,
  ...
  for (size_t i = 0; i < nd_; i++) {
    ...
    size_t min_diff =1e6, min_diff_dim = -1;
    for (size_t j = 0; j < dimension_; j++) {
      unsigned lnum = 0, rnum = 0;
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      if (query[div_dim_] - x[div_dim_] < -r_proportion * range_[div_dim_]) {
        ...
        MaxM = left_len + right_len + common_len;
      }
