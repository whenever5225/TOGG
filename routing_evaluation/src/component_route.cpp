...
//        std::cout << std::endl;
    ...
    void ComponentSearchRouteTOGGKDT::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
                                                std::vector<unsigned int> &res) {
        const auto L = index->getParam().get<unsigned>("L_search");
        const auto K = index->getParam().get<unsigned>("K_search");
        ...
        while (k < (int)L) {
            ...
            if (pool[k].flag) {
                ...
                unsigned right_len = index->Tn[n].right.size();
                // std::cout << "right_len: " << right_len << std::endl;
                unsigned common_len = index->Tn[n].common.size();
                std::vector<unsigned> nn;
                unsigned MaxM;
                if ((index->getQueryData() + index->getQueryDim() * query)[div_dim_] - (index->getBaseData() + index->getBaseDim() * n)[div_dim_] < -index->r_proportion * index->range_[div_dim_]) {
                    MaxM = left_len + common_len;
                    nn = index->Tn[n].left;
                }
                else if ((index->getQueryData() + index->getQueryDim() * query)[div_dim_] - (index->getBaseData() + index->getBaseDim() * n)[div_dim_] > index->r_proportion * index->range_[div_dim_]) {
                ...
                for (unsigned m = 0; m < common_len; m++) nn.push_back(index->Tn[n].common[m]);
        ...
        while (k < (int)L) {
            ...
            if (pool[k].flag) {
                ...
                unsigned common_len = index->Tn[n].common.size();
                std::vector<unsigned> nn;
                unsigned MaxM = left_len + right_len + common_len;

                for (unsigned m = 0; m < left_len; m++) nn.push_back(index->Tn[n].left[m]);
                for (unsigned m = 0; m < common_len; m++) nn.push_back(index->Tn[n].common[m]);
                for (unsigned m = 0; m < right_len; m++) nn.push_back(index->Tn[n].right[m]);
    ...
    void ComponentSearchRouteTOGGKMC::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
                                                std::vector<unsigned int> &res) {
        const auto L = index->getParam().get<unsigned>("L_search");
        const auto K = index->getParam().get<unsigned>("K_search");
        ...
        while (k < (int)L) {
            ...
            if (pool[k].flag) {
                ...
                for (unsigned i = 0; i < index->nc_[n].size(); i++) {
                    float dist = index->getDist()->compare(index->getQueryData() + query * index->getQueryDim(),
                                                           index->getBaseData() + index->nc_[n][i][0] * index->getBaseDim(),
                                                           (unsigned)index->getBaseDim());
                    index->addDistCount();
                    if (dist < pool[L - 1].distance) {
                        Index::Neighbor nn(index->nc_[n][i][0], dist, true);
                        int r = Index::InsertIntoPool(pool.data(), L, nn);

                        if (r < nk) nk = r;
                    }
                    if (dist < min_cluster_dist || min_cluster_dist == -1) {
                        min_cluster_dist = dist;
                        nn = index->nc_[n][i];
                        MaxM = index->nc_[n][i].size();
                    }
                }
        ...
        while (k < (int)L) {
            ...
            if (pool[k].flag) {
                ...
                for (unsigned i = 0; i < index->nc_[n].size(); i++) {
                    MaxM += index->nc_[n][i].size();
                    for (unsigned j = 0; j < index->nc_[n][i].size(); j++) {
                        nn.push_back(index->nc_[n][i][j]);
                    }
                }
                index->addHopCount();
                for (unsigned m = 0; m < MaxM; ++m) {
