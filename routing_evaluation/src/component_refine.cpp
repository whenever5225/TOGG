...
//            graphIndex.saveProperty(outIndexPath);
    ...
    void ComponentRefineGuided::RefineInner() {
    ...
    void ComponentRefineTOGGKDT::RefineInner() {
        SetConfigs();
        index->r_proportion = 1.0 / index->P;
        index->Tn.resize(index->getBaseLen());
    ...
    void ComponentRefineTOGGKDT::SetConfigs() {
        index->P = index->getParam().get<float>("P");
        index->P = index->P * index->P;
        std::cout << "P: " << index->P << std::endl;
    }

    void ComponentRefineTOGGKDT::get_range(float* &range) {
    float *max = new float[(size_t)index->getBaseDim()];
    float *min = new float[(size_t)index->getBaseDim()];
    range = new float[(size_t)index->getBaseDim()];
    for(size_t i = 0; i < index->getBaseDim(); i++) { //初始化为第一行数据
        max[i] = min[i] = index->getBaseData()[i];
    }
    for(size_t i = 0; i < index->getBaseDim(); i++) {
        for(size_t j = i; j < index->getBaseLen() * index->getBaseDim(); j += index->getBaseDim()) {
        if(max[i] < index->getBaseData()[j]) max[i] = index->getBaseData()[j];
        if(min[i] > index->getBaseData()[j]) min[i] = index->getBaseData()[j];
        }
    }
    for (size_t i = 0; i < index->getBaseDim(); i++) {
        range[i] = max[i] - min[i];
    ...
    void ComponentRefineTOGGKMC::RefineInner() {
        SetConfigs();
        ...
        for (size_t i = 0; i < index->getBaseLen(); i++) {
            ...
            if (size > index->cluster_num) {
                ...
                index->nc_[i].resize(tmp_c_num);
                
                std::set <size_t> :: iterator it = mset.begin();
                for (size_t j = 0; j < tmp_c_num || it != mset.end(); j++) {
                    float min_dist = -1;
                    unsigned* new_cluster_center = new unsigned[index->cluster_num];
                    std::memset(new_cluster_center, 0, sizeof(unsigned) * index->cluster_num);
                    for (size_t k = 0; k < size; k++) {
                        if (labels[k] == *it) {
                            float dist = index->getDist()->compare(index->getBaseData() + tmp_neighbor[k] * index->getBaseDim(), index->m_means[*it], index->getBaseDim());
                            if (dist < min_dist || min_dist == -1) {
                                min_dist = dist;
                                new_cluster_center[*it] = tmp_neighbor[k];
                            }
                        }
                    }
                    index->nc_[i][j].push_back(new_cluster_center[*it]); //新类中心作为该类的第一个元素
                    if (j != *it) {
                        for (size_t u = 0; u < size; u++) {
                            if (labels[u] == *it) {
                            labels[u] = j;
                ...
                for (size_t j = 0; j < size; j++) {
                    if (index->nc_[i][labels[j]][0] != tmp_neighbor[j]) {
                        index->nc_[i][labels[j]].push_back(tmp_neighbor[j]);
                    }
                }
                delete labels;
            }
    ...
    void ComponentRefineTOGGKMC::SetConfigs() {
        index->cluster_num = index->getParam().get<unsigned>("CN");
        index->m_means = new float*[index->cluster_num];
        for (size_t i = 0; i < index->cluster_num; i++) {
            index->m_means[i] = new float[index->getBaseDim()];
            std::memset(index->m_means[i], 0, sizeof(float) * index->getBaseDim());
        }
        index->m_maxIterNum = 100;
        index->m_endError = 0.001;
    }

    void ComponentRefineTOGGKMC::Cluster(unsigned* m_neighbors, int N, size_t *Label)
    {
        int size = 0;
        size = N;
        ...
        // Recursion
        std::vector <bool> cflag(index->cluster_num, 0);
        float* x = new float[index->getBaseDim()];	// Sample data
        ...
        int unchanged = 0;
        bool loop = true;
        int* counts = new int[index->cluster_num];	//每个类包含的数据点的个数
        float** next_means = new float*[index->cluster_num];	// New model for reestimation
        for(size_t i = 0; i < index->cluster_num; i++)
        {
            next_means[i] = new float[index->getBaseDim()];
        }
        while(loop)
        {
            //clean buffer for classification
            std::memset(counts, 0, sizeof(int) * index->cluster_num);
            for(size_t i = 0; i < index->cluster_num; i++)
            {
                std::memset(next_means[i], 0, sizeof(float) * index->getBaseDim());	//新的类中心的各维数据初始化为0
            }
            ...
            {
                for(size_t j = 0; j < index->getBaseDim(); j++)
                    x[j] = *(index->getBaseData() + (*(m_neighbors + i)) * index->getBaseDim() + j);

                currCost += GetLabel(x, &label);	//到所属类的最小距离
                counts[label]++;
                for(size_t d = 0; d < index->getBaseDim(); d++)	//属于某个类的所有数据点相应维的值累加
                {
            ...
            // Reestimation
            for(size_t i = 0; i < index->cluster_num; i++)	//重新评估类中心
            {
                if(counts[i] > 0)
                {
                    for(size_t d = 0; d < index->getBaseDim(); d++)
                    {
                        next_means[i][d] /= counts[i];
                    }
                    std::memcpy(index->m_means[i], next_means[i], sizeof(float) * index->getBaseDim());
                }
            }
            // Terminal conditions
            iterNum++;
            if(fabs(lastCost - currCost) < index->m_endError * lastCost)
            {
                unchanged++;
            }
            if(iterNum >= index->m_maxIterNum || unchanged >= 3)
            {
                loop = false;
            }
    ...
    void ComponentRefineTOGGKMC::Init(unsigned* m_neighbors, int N)	//初始化类中心
    {
        int size = N;

        float* sample = new float[index->getBaseDim()];
            for(size_t i = 0; i < index->cluster_num; i++)
            {
                int select = i * size / index->cluster_num;
                for (size_t j = 0; j < index->getBaseDim(); j++) {
                    sample[j] = *(index->getBaseData() + (*(m_neighbors + select)) * index->getBaseDim() + j);
                }
                std::memcpy(index->m_means[i], sample, sizeof(float) * index->getBaseDim());
            }
    }

    float ComponentRefineTOGGKMC::GetLabel(const float* sample, int* label)
    {
        float dist = -1;
    
    
        for(size_t i = 0; i < index->cluster_num; i++)	//计算sample到每个类中心的距离
        {
        float temp = index->getDist()->compare(sample, index->m_means[i], index->getBaseDim());
            if(temp < dist || dist == -1)
            {
                dist = temp;
                *label = i;
