...
namespace weavess {
    ...
    void ComponentSearchEntryGuidedCentroid::SearchEntryInner(unsigned int query, std::vector<Index::Neighbor> &pool) {
        const auto L = index->getParam().get<unsigned>("L_search");
        pool.resize(L + 1);
        std::vector<unsigned> init_ids(L);
        boost::dynamic_bitset<> flags{index->getBaseLen(), 0};
        ...
        unsigned i = 0, j = 0;
        for (; i < index->Tn[index->ep_].left.size() && j < index->Tn[index->ep_].right.size(); i++, j++) {
            tmp.push_back(index->Tn[index->ep_].left[i]);
            tmp.push_back(index->Tn[index->ep_].right[j]);
        }
        while (i < index->Tn[index->ep_].left.size()) {tmp.push_back(index->Tn[index->ep_].left[i]); i++;}
        while (j < index->Tn[index->ep_].right.size()) {tmp.push_back(index->Tn[index->ep_].right[j]); j++;}

        unsigned tmp_l = 0;
        for (; tmp_l < L && tmp_l < tmp.size(); tmp_l++) {
            init_ids[tmp_l] = tmp[tmp_l];
            flags[init_ids[tmp_l]] = true;
        }
    ...
    void ComponentSearchEntryTOGGKDTCentroid::SearchEntryInner(unsigned int query, std::vector<Index::Neighbor> &pool) {
        const auto L = index->getParam().get<unsigned>("L_search");
        pool.resize(L + 1);
        std::vector<unsigned> init_ids(L);
        boost::dynamic_bitset<> flags{index->getBaseLen(), 0};
        ...
        for (; i < index->Tn[index->ep_].left.size() && j < index->Tn[index->ep_].right.size() && k < index->Tn[index->ep_].common.size(); i++, j++, k++) {
            tmp.push_back(index->Tn[index->ep_].left[i]);
            tmp.push_back(index->Tn[index->ep_].right[j]);
            tmp.push_back(index->Tn[index->ep_].common[k]);
        }
        while (i < index->Tn[index->ep_].left.size()) {tmp.push_back(index->Tn[index->ep_].left[i]); i++;}
        while (j < index->Tn[index->ep_].right.size()) {tmp.push_back(index->Tn[index->ep_].right[j]); j++;}
        while (k < index->Tn[index->ep_].common.size()) {tmp.push_back(index->Tn[index->ep_].common[k]); k++;}
    ...
    void ComponentSearchEntryTOGGKMCCentroid::SearchEntryInner(unsigned int query, std::vector<Index::Neighbor> &pool) {
        const auto L = index->getParam().get<unsigned>("L_search");
        pool.resize(L + 1);
        std::vector<unsigned> init_ids(L);
        boost::dynamic_bitset<> flags{index->getBaseLen(), 0};
        ...
        std::vector<unsigned> tmp;
        unsigned i = 0, j = 0, k = 0;
        unsigned cnum = index->nc_[index->ep_].size();
        for (unsigned i = 0; i < L; i++) {
            for (unsigned j = 0; j < index->nc_[index->ep_].size(); j++) {
                if (i < index->nc_[index->ep_][j].size()) {
                    tmp.push_back(index->nc_[index->ep_][j][i]);
                }
            }
        }
