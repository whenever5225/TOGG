...
{
    if (argc < 3 || argc > 5) {
      std::cout << "./evaluation dataset exc_type [K] [L]"
                << std::endl;
      exit(-1);
    }
    ...
    std::string graph_file("hnsw_" + dataset + ".graph");
      ...
      if (L_type == "L_SEARCH_ASCEND") {
        ...
        for (unsigned i = 0; i < iter_num; i++) {
          ...
          for (int i = 0; i < query_num; i++) {
            ...
            index2.SearchByVector(q, K, L, result[i]);
          }
          auto e1 = std::chrono::high_resolution_clock::now();
          std::chrono::duration<double> diff = e1 - s1;
          std::cout << "search time: " << diff.count() << "\n";
          size_t DistCount = index2.GetDistCount();
