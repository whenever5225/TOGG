...
namespace efanna2e {
    ...
    static void GenRandom(std::mt19937 &rng, unsigned *addr, unsigned size, unsigned N) {
        for (unsigned i = 0; i < size; ++i) {
            addr[i] = rng() % (N - size); //0 ～ N - size - 1
        }
        std::sort(addr, addr + size);
        for (unsigned i = 1; i < size; ++i) { //保证各元素均不相等，addr[i]的范围0 ～ N - 2
            if (addr[i] <= addr[i - 1]) {
                addr[i] = addr[i - 1] + 1;
            }
        }
        unsigned off = rng() % N; //off的取值范围是0 ~ N - 1
        for (unsigned i = 0; i < size; ++i) {
            addr[i] = (addr[i] + off) % N;  //addr[i]的各元素取值范围是 0 ~ N - 1且各元素都不相等
        }
    }
