...
struct nhood{
  ...
  nhood(){}
  //参数初始化
  nhood(unsigned l, unsigned s, std::mt19937 &rng, unsigned N){ //N为数据的个数
    M = s;
    nn_new.resize(s * 2);
    GenRandom(rng, &nn_new[0], (unsigned)nn_new.size(), N);  //随机给各数据点初始化 2 * s 个邻居
    // nn_new.reserve(s * 2);
    pool.reserve(l);
  }
  //用一个nhood给另一个nhood初始化
  nhood(const nhood &other){
    M = other.M;
    std::copy(other.nn_new.begin(), other.nn_new.end(), std::back_inserter(nn_new));  //使用push_back()在nn_new容器尾端安插元素
    nn_new.reserve(other.nn_new.capacity());
    pool.reserve(other.pool.capacity());
  }
  void insert (unsigned id, float dist) { //仅考虑距离因素
    LockGuard guard(lock);
    if (dist > pool.front().distance) return; //dist大于pool中起始元素的distance
    for(unsigned i=0; i<pool.size(); i++){  //已存在
      if(id == pool[i].id)return;
    }
    if(pool.size() < pool.capacity()){
      pool.push_back(Neighbor(id, dist, true));
      std::push_heap(pool.begin(), pool.end()); //指定区间最后一个元素入堆，https://blog.csdn.net/qq_34793133/article/details/81222919
    }else{
      std::pop_heap(pool.begin(), pool.end());  //弹出heap顶元素, 将其放置于区间末尾
