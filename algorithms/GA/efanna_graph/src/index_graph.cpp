...
IndexGraph::IndexGraph(const size_t dimension, const size_t n, Metric m, Index *initializer)
    : Index(dimension, n, m),
      initializer_{initializer} { //initializer_为Index类型的实例对象,(IndexRandom)
...
#pragma omp parallel for
  for(unsigned i=0; i<c.size(); i++){
    ...
    std::partial_sort(tmp.begin(), tmp.begin() + _CONTROL_NUM, tmp.end());  //部分排序
...
void IndexGraph::InitializeGraph(const Parameters &parameters) {
  ...
  const unsigned S = parameters.Get<unsigned>("S"); //限制邻居的个数

  graph_.reserve(nd_);
  std::mt19937 rng(rand());
  for (unsigned i = 0; i < nd_; i++) {  //给每个数据点随机产生 2 * S 个 id 存于nn_new里面
...
#pragma omp parallel for
  for (unsigned i = 0; i < nd_; i++) {
    ...
    initializer_->Search(query, data_, S + 1, parameters, tmp.data());  //data()返回一个指向数组的指针
    //搜索结果存放在tmp中
    ...
    std::make_heap(graph_[i].pool.begin(), graph_[i].pool.end()); //大根堆结构存储每个数据点的邻居池
...
void IndexGraph::Build(size_t n, const float *data, const Parameters &parameters) {
  ...
  data_ = data;
  assert(initializer_->HasBuilt()); //执行indexRandom初始化时，has_built已为true
...
void IndexGraph::Search(
    const float *query, //查询点
    const float *x, //载入的原始数据集
    size_t K, //S + 1
    const Parameters &parameter,
    unsigned *indices) {  //随机初始化入口点搜索
  const unsigned L = parameter.Get<unsigned>("L_s");
  std::cout << "L_search: " << L << "\n";
  ...
  std::sort(retset.begin(), retset.begin()+L);  //按距离query的距离升序排序
  int k=0;
  while(k < (int)L) {
    int nk = L;
...
void IndexGraph::Save(const char *filename) {
  ...
  assert(final_graph_.size() == nd_); //nd_为数据个数
