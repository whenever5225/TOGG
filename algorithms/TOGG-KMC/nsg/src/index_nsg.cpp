...
void IndexNSG::InitClusterPara(const size_t cl_num) {
  cluster_num = cl_num;
...
void IndexNSG::Load(const char *filename) {
  ...
  while (!in.eof()) {
   unsigned cluster_num_;
    ...
    if (cluster_num < cluster_num_ || cluster_num == 0){
...
void IndexNSG::Build(size_t n, const float *data, const Parameters &parameters) {
  ...
  nc_.resize(nd_);
  SetInitMode(IndexNSG::InitUniform);
...
void IndexNSG::Cluster(unsigned* m_neighbors, int N, size_t *Label)
{
	int size = 0;
	size = N;
...
void IndexNSG::Init(unsigned* m_neighbors, int N)	//初始化类中心
{
	int size = N;
...
float IndexNSG::GetLabel(const float* sample, int* label)	//返回sample到所有类中心的最小距离
{
	float dist = -1;
