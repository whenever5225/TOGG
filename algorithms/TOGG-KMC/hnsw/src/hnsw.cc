...
void Hnsw::Fit() {
    ...
    // memory_per_node_higher_level_ = sizeof(int) * (5 + MaxM_);  // "1" for saving num_links
    // long long higher_level_size = memory_per_node_higher_level_ * totalLevel;
    // memory_per_data_ = sizeof(float) * data_dim_;
    // memory_per_link_level0_ = sizeof(int) * (1 + 5 + MaxM0_);  // "1" for offset pos, 1" for saving num_links
...
void Hnsw::Partition_neighbors() {
    ...
    num_nodes_ = nodes_.size();
    long long model_config_size = GetModelConfigSize();
    memory_per_node_higher_level_ = sizeof(int) * (1 + cluster_num + MaxM_);  // "1" for saving num_cluster
    long long higher_level_size = memory_per_node_higher_level_ * totalLevel;
    memory_per_data_ = sizeof(float) * data_dim_;
    ...
    for (size_t i = 0; i < nodes_.size(); ++i) {
        ...
        if(level > 0) {
            nodes_[i]->CopyDataAndLevel0LinksToOptIndex(model_level0_ + i * memory_per_node_level0_, higher_offset, MaxM0_);
            nodes_[i]->CopyHigherLevelLinksToOptIndex(model_higher_level_ + memory_per_node_higher_level_*higher_offset, memory_per_node_higher_level_, cluster_num, m_means);
            higher_offset += nodes_[i]->GetLevel();
        } else {
            nodes_[i]->CopyDataAndLevel0LinksToOptIndex(model_level0_ + i * memory_per_node_level0_, 0, MaxM0_);
        }
...
void Hnsw::SearchById_true(int cur_node_id, float cur_dist, const float* qraw, unsigned int* visited, priority_queue<FurtherFirstNew>& res, priority_queue<CloserFirstNew>& candidates, size_t k, size_t ef_search, vector<pair<int, float> >& result) {
    ...
    while(!candidates.empty()) {
        ...
        float *cur_node_id_data = (float *)(model_level0_ + cur_node_id*memory_per_node_level0_ + memory_per_link_level0_);
    ...
    // std::cout<<"comparison_times = "<<comparison_times_<<endl;
...
void Hnsw::SearchById_(int cur_node_id, float cur_dist, const float* qraw, size_t k, size_t ef_search, vector<pair<int, float> >& result) {
    ...
    while(!candidates.empty()) {
        const CloserFirstNew& cand = candidates.top();
        float lowerbound = res.top().GetDistance();
        if (cand.GetDistance() > lowerbound && res.size() == ef_search) break;
        int cur_node_id = cand.GetId();
        int *data = (int*)(model_level0_ + cur_node_id*memory_per_node_level0_ + sizeof(int));
        int size = *data;
        candidates.pop();
...
//     std::cout<<"比较次数："<<comparison_times<<endl;
...
void Hnsw::SearchByVector(const vector<float>& qvec, size_t k, size_t ef_search, size_t alg, vector<pair<int, float>>& result) {
    ...
    float *cur_node_id_data = (float *)(model_level0_ + cur_node_id*memory_per_node_level0_ + memory_per_link_level0_);
    float cur_dist = dist_cls_->Evaluate(qraw, cur_node_id_data, data_dim_, TmpRes);
    comparison_times_ += 1;
    ...
    for (int i = maxlevel; i > 0; --i) {
        ...
        while (changed) {
            ...
            int offset = *((int*)(level_offset));   //定位在高层的起始位置
            ...
            int csize = *data;
            int size = 0;
            std::vector <int> cnum(csize);
            for (int j = 0; j < csize; j++) {
                data++;
                cnum[j] = *data;
            }
            int cluster_offset = 1;
            float min_cluster_dist = -1;
            int *op_cluster = NULL;
            for (int i = 0; i < csize; i++) {
                int mid = *(data + cluster_offset);
                float temp = dist_cls_ -> Evaluate(qraw, (const float*)(model_level0_ + mid * memory_per_node_level0_ + memory_per_link_level0_), data_dim_, TmpRes);  //2019.11.13
                visited[mid] = mark;
                comparison_times_ += 1;
                if(res.size() < ef_search || temp < res.top().GetDistance()){
                    res.emplace(mid, temp);
                }
                if(candidates.size() < ef_search || temp < res.top().GetDistance()){
                    candidates.emplace(mid, temp);
                }
                if (temp < cur_dist) {
                    cur_dist = temp;
                    cur_node_id = mid;
                    cur_node_id_data = (float *)(model_level0_ + cur_node_id*memory_per_node_level0_ + memory_per_link_level0_);
                ...
                op_cluster = data + cluster_offset;
                size = cnum[i];
                }
...
void Hnsw::InitClusterPara(const size_t cl_num) {
  cluster_num = cl_num;
  std::cout << "cn: " << cluster_num << "\n";
  m_means = new float*[cluster_num];
  for (size_t i = 0; i < cluster_num; i++) {
    m_means[i] = new float[data_dim_];
    std::memset(m_means[i], 0, sizeof(float) * data_dim_);
  }
}
