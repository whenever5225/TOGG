...
public:
    ...
    void CopyHigherLevelLinksToOptIndex(char* mem_offset, long long memory_per_node_higher_level, float r_proportion_, const float* range_);
    void CopyDataAndLevel0LinksToOptIndex(char* mem_offset, int higher_level_offset, int M0);
    void NeighborsTree(std::vector<HnswNode*> &neighbors, int &div_dim, int &l_size, int &c_size, int &r_size, float r_proportion_, const float* range_);
...
private:
    void CopyLinksToOptIndex(char* mem_offset, int level, float r_proportion_, const float* range_);
