...
void Hnsw::Partition_neighbors() {
    ...
    num_nodes_ = nodes_.size();
    long long model_config_size = GetModelConfigSize();
    memory_per_node_higher_level_ = sizeof(int) * (4 + MaxM_);  // "1" for saving num_links
    long long higher_level_size = memory_per_node_higher_level_ * totalLevel;
    memory_per_data_ = sizeof(float) * data_dim_;
    ...
    for (size_t i = 0; i < nodes_.size(); ++i) {
        ...
        if(level > 0) {
            nodes_[i]->CopyDataAndLevel0LinksToOptIndex(model_level0_ + i * memory_per_node_level0_, higher_offset, MaxM0_);
            nodes_[i]->CopyHigherLevelLinksToOptIndex(model_higher_level_ + memory_per_node_higher_level_*higher_offset, memory_per_node_higher_level_, r_proportion_, range_);
            higher_offset += nodes_[i]->GetLevel();
        } else {
            nodes_[i]->CopyDataAndLevel0LinksToOptIndex(model_level0_ + i * memory_per_node_level0_, 0, MaxM0_);
        }
...
bool Hnsw::LoadModel(const string& fname, const bool use_mmap) {
    ...
    ptr = GetValueAndIncPtr<float>(ptr, r_proportion_);
...
void Hnsw::SearchByVector(const vector<float>& qvec, size_t k, size_t ef_search, size_t alg, vector<pair<int, float>>& result) {
    ...
    for (int i = maxlevel; i > 0; --i) {
        ...
        while (changed) {
            ...
            int size;
            int div_dim = *data;data++;
            int l_size = *data;data++;
            int c_size = *data;data++;
            int r_size = *data;

            if ((*(qraw + div_dim) -  *(cur_node_id_data + div_dim)) < -r_proportion_ * range_[div_dim]) {
                size = l_size + c_size;
            }
            else if ((*(qraw + div_dim) - *(cur_node_id_data + div_dim)) > r_proportion_ * range_[div_dim]) {
                size = r_size + c_size;
                data += l_size;
            }
            else {
                size = l_size + c_size + r_size; //方案1
                // size = c_size; //方案2
                // data += l_size;
            }
...
void Hnsw::SaveModelConfig(char* ptr) {
    ...
    ptr = SetValueAndIncPtr<float>(ptr, r_proportion_);
}
...
void Hnsw::Load_range(float* r) {
  range_ = r;
}

void Hnsw::InitRangeProportion(const float rr_p) {
  r_proportion_ = 1.0 / rr_p;
  std::cout << "rp: " << r_proportion_ << std::endl;
}
