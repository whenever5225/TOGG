...
void IndexNSG::SaveNSG(const char *filename) { //保存nsg图
...
void IndexNSG::SaveNSG_TOGG_KDT(const char *filename) { //保存nsg图
  std::ofstream out(filename, std::ios::binary | std::ios::out);
  assert(Tn.size() == nd_);

  out.write((char *)&width, sizeof(unsigned));
  out.write((char*)&r_proportion, sizeof(float));
  out.write((char *)&ep_, sizeof(unsigned));
  for (unsigned i = 0; i < nd_; i++) {
    out.write((char *)&Tn[i].div_dim, sizeof(unsigned));
    unsigned left_len = (unsigned)Tn[i].left.size();
    unsigned common_len = (unsigned)Tn[i].common.size();
    unsigned right_len = (unsigned)Tn[i].right.size();
    out.write((char *)&left_len, sizeof(unsigned));
    out.write((char *)&common_len, sizeof(unsigned));
    out.write((char *)&right_len, sizeof(unsigned));
    out.write((char *)Tn[i].left.data(), left_len * sizeof(unsigned));
    out.write((char *)Tn[i].common.data(), common_len * sizeof(unsigned));
    out.write((char *)Tn[i].right.data(), right_len * sizeof(unsigned));
  }
  out.close();
}

void IndexNSG::SaveGuided(const char *filename) { //保存nsg图
  std::ofstream out(filename, std::ios::binary | std::ios::out);
  assert(Tn.size() == nd_);
...
void IndexNSG::Load(const char *filename) { //载入nsg_TOGG_KDT图
  std::ifstream in(filename, std::ios::binary);
  in.read((char *)&width, sizeof(unsigned));
  in.read((char*)&r_proportion, sizeof(float));
  in.read((char *)&ep_, sizeof(unsigned));
  // width=100;
  unsigned cc = 0;
  while (!in.eof()) {
    unsigned left_len, common_len, right_len;
    Tnode tmp;
    ...
    if (in.eof()) break;
    cc += (left_len + right_len + common_len);
...
void IndexNSG::Load_nsg_graph(const char *filename) {
  std::ifstream in(filename, std::ios::binary);
  in.read((char *)&width, sizeof(unsigned));
  in.read((char *)&ep_, sizeof(unsigned));
  // width=100;
...
void IndexNSG::LoadGuided(const char *filename) { //载入nsg_TOGG_KDT图
  std::ifstream in(filename, std::ios::binary);
  in.read((char *)&width, sizeof(unsigned));
  in.read((char *)&ep_, sizeof(unsigned));
  // width=100;
...
void IndexNSG::Load_range(const float* r) {
  range_ = r;
}
void IndexNSG::InitRangeProportion(const float rr_p) {
  r_proportion = 1.0 / rr_p;
  std::cout << "rp: " << r_proportion << std::endl;
}
...
void IndexNSG::BuildOriginalNSG(size_t n, const float *data, const Parameters &parameters) {
  ...
  for (size_t i = 0; i < nd_; i++) {
    ...
    for (size_t j = 0; j < dimension_; j++) {
      ...
      for (size_t k = 0; k < size; k++) {
        if (*(data_ + final_graph_[i][k] + j) - *(data_ + i + j) < -r_proportion * range_[j]) {
          lnum++;
        }
        else if (*(data_ + final_graph_[i][k] + j) - *(data_ + i + j) > r_proportion * range_[j]) {
          rnum++;
        }
      }
    ...
    // std::cout << "min_diff_dim: " << min_diff_dim << std::endl;
    // Tnode Tn_tmp;
    Tn[i].div_dim = min_diff_dim;
    for (size_t k = 0; k < size; k++) {
      if (*(data_ + final_graph_[i][k] + min_diff_dim) - *(data_ + i + min_diff_dim) < -r_proportion * range_[min_diff_dim]) {
        Tn[i].left.push_back(final_graph_[i][k]);
      }
      else if (*(data_ + final_graph_[i][k] + min_diff_dim) - *(data_ + i + min_diff_dim) >= r_proportion * range_[min_diff_dim]) {
        Tn[i].right.push_back(final_graph_[i][k]);
        // if (i == 0) {
        //   std::cout << Tn[i].right[i] << " " << final_graph_[i][k] << std::endl;
        // }
        // cr++;
      }
      else {
        Tn[i].common.push_back(final_graph_[i][k]);
      }
    }
    // Tn.push_back(Tn_tmp);
    // if (i == 0 || i == nd_ - 1) {
    //   for (size_t k = 0; k < Tn[i].left.size(); k++) {
    //     std::cout << Tn[i].left[k];
    //     if (k != Tn[i].left.size() - 1) {
    //       std::cout << " ";
    //     }
    //     else {
    //       std::cout << std::endl;
    ...
    //   std::cout << "left_len: " << Tn[i].left.size() << " " << "right_len: " << Tn[i].right.size() << std::endl;
    // }
    max = max < size ? size : max;
    min = min > size ? size : min;
    avg += size;
...
void IndexNSG::SearchOptimalP(size_t n, const float *data, const Parameters &parameters) {
  std::string nsg_graph_path = parameters.Get<std::string>("nsg_graph_path");
  // unsigned range = parameters.Get<unsigned>("R");
  Load_nsg_graph(nsg_graph_path.c_str());
  data_ = data;
  // init_graph(parameters);
  // SimpleNeighbor *cut_graph_ = new SimpleNeighbor[nd_ * (size_t)range];
  // Link(parameters, cut_graph_);
...
void IndexNSG::BuildGuided(size_t n, const float *data, const Parameters &parameters) {
  std::string nsg_graph_path = parameters.Get<std::string>("nsg_graph_path");
  // unsigned range = parameters.Get<unsigned>("R");
  Load_nsg_graph(nsg_graph_path.c_str());
  data_ = data;
  ...
  for (size_t i = 0; i < nd_; i++) {
    ...
    for (size_t j = 0; j < dimension_; j++) {
      int lnum = 0, rnum = 0;
      for (size_t k = 0; k < size; k++) {
        if (*(data_ + final_graph_[i][k] + j) - *(data_ + i + j) < 0) {
          lnum++;
        }
        else {
          rnum++;
    ...
    Tn[i].div_dim = min_diff_dim;
    for (size_t k = 0; k < size; k++) {
      if (*(data_ + final_graph_[i][k] + min_diff_dim) - *(data_ + i + min_diff_dim) < 0) {
        Tn[i].left.push_back(final_graph_[i][k]);
      }
      else {
        Tn[i].right.push_back(final_graph_[i][k]);
...
void IndexNSG::SearchWithOptGraph(const float *query, size_t K,
  ...
  neighbors++;  //跳过用于划分的维度
  unsigned MaxM_ep = *neighbors;  //左子树邻居个数
  neighbors++;
  MaxM_ep += *neighbors;  //公共子树
  neighbors++;
  MaxM_ep += *neighbors;  //加上右子树邻居个数
  neighbors++;
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      unsigned *x = (unsigned *)(opt_graph_ + node_size * n);
      x++;
      unsigned *neighbors = (unsigned *)(opt_graph_ + node_size * n + data_len);
      unsigned div_dim_ = *neighbors;
      // std::cout << "div_dim: " << div_dim << std::endl;
      unsigned left_len = *(neighbors + 1);
      unsigned common_len = *(neighbors + 2);
      unsigned right_len = *(neighbors + 3);
      // std::cout << "right_len: " << right_len << std::endl;
      neighbors += 4;
      unsigned MaxM;
      if (query[div_dim_] - x[div_dim_] < -r_proportion * range_[div_dim_]) {
        MaxM = left_len + common_len;
      }
      else if (query[div_dim_] - x[div_dim_] > r_proportion * range_[div_dim_]) {
        MaxM = right_len + common_len;
        neighbors += left_len;
      }
      else {
        MaxM = common_len;
      ...
      for (unsigned m = 0; m < MaxM; ++m) {
        // std::cout << m + 1 << " " << neighbors[m] << std::endl;
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      x++;
      unsigned *neighbors = (unsigned *)(opt_graph_ + node_size * n + data_len);
      unsigned MaxM = *(neighbors + 1) + *(neighbors + 2) + *(neighbors + 3);
      neighbors += 4;
      for (unsigned m = 0; m < MaxM; ++m)
        _mm_prefetch(opt_graph_ + node_size * neighbors[m], _MM_HINT_T0);
      for (unsigned m = 0; m < MaxM; ++m) {
...
void IndexNSG::SearchWithOptGuidedGraph(const float *query, size_t K,
                                  const Parameters &parameters, unsigned *indices) {
  unsigned L = parameters.Get<unsigned>("L_search");
  DistanceFastL2 *dist_fast = (DistanceFastL2 *)distance_;
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      // std::cout << "div_dim: " << div_dim << std::endl;
      unsigned left_len = *(neighbors + 1);
      unsigned right_len = *(neighbors + 2);
      // std::cout << "right_len: " << right_len << std::endl;
      neighbors += 3;
      unsigned MaxM;
      if (query[div_dim_] - x[div_dim_] < 0) {
        MaxM = left_len;
      }
      else {
        MaxM = right_len;
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      x++;
      unsigned *neighbors = (unsigned *)(opt_graph_ + node_size * n + data_len);
      unsigned MaxM = *(neighbors + 1) + *(neighbors + 2);
      neighbors += 3;
      for (unsigned m = 0; m < MaxM; ++m)
        _mm_prefetch(opt_graph_ + node_size * neighbors[m], _MM_HINT_T0);
      for (unsigned m = 0; m < MaxM; ++m) {
...
void IndexNSG::OptimizeGraph(float *data) {  // use after build or load
  ...
  neighbor_len = (width + 4) * sizeof(unsigned);
  ...
  for (unsigned i = 0; i < nd_; i++) {
    ...
    unsigned div_dim_ = Tn[i].div_dim;
    unsigned left_len = Tn[i].left.size();
    unsigned common_len = Tn[i].common.size();
    unsigned right_len = Tn[i].right.size();
    std::memcpy(cur_node_offset, &div_dim_, sizeof(unsigned));
    cur_node_offset += sizeof(unsigned);
    std::memcpy(cur_node_offset, &left_len, sizeof(unsigned));
    cur_node_offset += sizeof(unsigned);
    std::memcpy(cur_node_offset, &common_len, sizeof(unsigned));
    cur_node_offset += sizeof(unsigned);
    std::memcpy(cur_node_offset, &right_len, sizeof(unsigned));
    cur_node_offset += sizeof(unsigned);
    std::memcpy(cur_node_offset, Tn[i].left.data(),
                left_len * sizeof(unsigned));
    cur_node_offset += left_len * sizeof(unsigned);
    std::memcpy(cur_node_offset, Tn[i].common.data(),
                common_len * sizeof(unsigned));
    cur_node_offset += common_len * sizeof(unsigned);
    std::memcpy(cur_node_offset, Tn[i].right.data(),
                right_len * sizeof(unsigned));
...
void IndexNSG::OptimizeGuidedGraph(float *data) {  // use after build or load

  data_ = data;
  data_len = (dimension_ + 1) * sizeof(float);
  neighbor_len = (width + 3) * sizeof(unsigned);
  node_size = data_len + neighbor_len;
  opt_graph_ = (char *)malloc(node_size * nd_);
  DistanceFastL2 *dist_fast = (DistanceFastL2 *)distance_;
  for (unsigned i = 0; i < nd_; i++) {
